class DOMmodifier{
    ...
    changePriorityVarStyle(itemDOM, priority){
        ...
        priorityDOM.innerText = parseInt(priority) ? priority : 'nesvarbu';
        priorityDOM.style.cssText = parseInt(priority) ? 'color:#000; font-style:normal;': 'color:#999; font-style:italic;';
    }
    ...
    changeContent(itemDOM, data){
        ...
        if(listDOMS && listDOMS.length > 0){
            listDOMS.forEach(listDOM => {
                ...
                let html = '';
                currList.forEach(item => html += `<li data-id="">${item}</li>`)
